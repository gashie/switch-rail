...
- [ ] B1.1 — Repo init, package.json, scripts, .env.example, README
...
- [ ] B2.7 — Bulk file adapter (CSV, XLSX, pain.001 — line-by-line to envelopes)
...
- [ ] B6.1 — `modules/fraud/` (rules engine: velocity, geo, time, amount, new-beneficiary)
...
- [ ] B6.7 — `modules/fast-track-reversal/` (PIX-MED equivalent — defined-window fraud clawback)
...
- [ ] B8.1 — `modules/overlays-r2p/` (Request to Pay, payer authorization)
- [ ] B8.2 — `modules/overlays-qr/` (EMVCo static + dynamic QR)
...
- **Current block:** B1.1 (not started)
- **Tests passing:** 0
